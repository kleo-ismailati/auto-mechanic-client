...
export class BookingDetailsPageComponent implements OnInit {
  ...
  onAddRepair(repair: RepairCreate | null) {
    this.modalService.dismissAll();
    if (repair) {
      this.bookingManagementService.postNewRepair(this.booking.id!, repair).subscribe(
        () => {
  ...
  onUpdateBooking(booking: Booking) {
    this.bookingManagementService.postUpdatedBooking(this.booking.id!, booking).subscribe(
      () => {
  ...
  onUpdateRepair(repair: Repair) {
    this.bookingManagementService.postUpdatedRepair(repair.id, repair).subscribe(
      () => {
  ...
  onConfirmRepairDelete(isConfirmed: boolean) {
    this.modalService.dismissAll();
    if (isConfirmed) {
      this.bookingManagementService.deleteRepair(this.idOfRepairToDelete).subscribe(
        () => {
          this.alertService.warn("Repair with id " + this.idOfRepairToDelete + " was deleted!", {autoClose: true});
          window.scroll({
            top: 0,
            left: 0,
            behavior: 'smooth'
