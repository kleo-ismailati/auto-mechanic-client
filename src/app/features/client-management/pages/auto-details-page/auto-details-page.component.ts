...
export class AutoDetailsPageComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.clientManagementService.getAuto(this.autoId).subscribe(
      (data: Auto) => {
        this.auto = data;
        if (this.auto.imageId != null) {
          this.clientManagementService.getImage(this.auto.imageId, this.headers).subscribe(
            (image) => {
  ...
  onUpdateAuto(auto: Auto) {
    this.clientManagementService.updateAuto(this.autoId, auto).subscribe(
      () => {
  ...
  onChangeImage(image: FormData) {
    this.clientManagementService.setImageForAuto(this.autoId, image).subscribe(
      {
        next: () => {
  ...
  onAddBooking(booking: BookingCreate | null) {
    ...
    if (booking) {
      ...
      this.clientManagementService.createBooking(booking).subscribe(
        () => {
