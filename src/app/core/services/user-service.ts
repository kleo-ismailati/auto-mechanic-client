...
export class UserService {
  ...
  logoutUserOnExpire(){
    ...
    this.router.navigate([""]).then(()=>{
      this.alertService.warn("User session expired! Please sign in again!", { autoClose: false, keepAfterRouteChange: true });
    })
  }
  ...
  logoutUserForced(){
    ...
    this.router.navigate([""]).then(()=>{
      this.alertService.warn("User session invalid! Please sign in again!", { autoClose: true, keepAfterRouteChange: true });
    })
  }
  ...
  autoLogoutUser(){
    ...
    if (expireDate && expireDate.getTime() < currentDate.getTime()) {
      ...
      this.currentUserSubject.next(null);
      this.router.navigate([""]).then(()=>{
        this.alertService.warn("User session expired! Please sign in again!", { autoClose: true, keepAfterRouteChange: true });
      })
    }
  }
